...
for i in range(len(test[0])):
    ...
    data = np.zeros((3, nFrames, IMAGE_SIZE, IMAGE_SIZE), dtype=np.float32)
    ...
    pred = output.data[0]
    pred = np.exp(pred) / torch.sum(np.exp(pred))
    argsort_pred = np.asarray(np.argsort(-pred))[0:10]

    label = test[1][index]
    print(label, argsort_pred)
...
output.to_csv("HW9/Part_2/Output/SequenceFrames_test.csv")
full_pred.to_csv("HW9/Part_2/Output/SequenceFrames_predictions.csv")
np.save('sequence_frames_confusion_matrix.npy', confusion_matrix)
np.save('HW9/Part_2/Output/sequence_frames_confusion_matrix.npy', confusion_matrix)

...
model = ResNet(block=Residual_block, channel_first=32, channel_per_layer=[32,64,128,256], 
               block_per_layer=[2,4,4,2], stride_per_layer=[1,2,2,2],dropout_rate=0.2).to(device)
...
for epoch in range(num_epochs):
    ...
    print("Epoch[{}/{}], Loss {:4f}, Accuracy {:4f}%".format(
        epoch+1, num_epochs, loss.item(), train_acc_vec[epoch]))
...
# Save accuracy
torch.save({"train_acc":train_acc_vec, "test_acc": test_acc_vec}, "AccCheckpoint.pth.tar")

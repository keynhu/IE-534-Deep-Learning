...
from utils import init_embedding, load_embeddings, clip_gradient, save_checkpoint, adjust_learning_rate, compute_accuracy, AverageMeter
...
h5data_folder = "scratch" # Save preprocessed captions and image paths
...
num_epochs = 30 # Not important, because it will usually strike the walltime before finishing this
workers = 1
encoder_lr = 1e-4
decoder_lr = 1e-4
...
batch_size = 32 # May need to change to 64, because 32 is too slow
#batch_size = 64 # May need to shrink to 32, in case of out-of-memory error
...
def main():
    ...
    if checkpoint is None: # No pretrained model, set model from beginning
        ...
        if fine_tune_encoder:
            for param in encoder_param:
                ...
                dist.all_reduce(tensor0, op=dist.reduce_op.SUM)
                param.data = tensor0/np.sqrt(np.float(num_nodes))
        encoder_optimizer = optim.Adam(params = encoder_param, lr = encoder_lr) if fine_tune_encoder else None
    else:
    ...
    train_set = CaptionDataset(data_folder = h5data_folder, data_name = dataset_name,
                               split = "TRAIN", transform = transforms.Compose([normalize]))
        ...
        for j, (images, captions, caplens, all_caps) in enumerate(val_loader):
            ...
            if (j+1) % print_freq == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Top-5 Accuracy {top5.val:.3f} ({top5.avg:.3f})'.format(epoch+1, j, len(val_loader),
                                                                          batch_time=batch_time,
                                                                          data_time=data_time, loss=losses,
                                                                          top5=top5accs))
                now_time = datetime.datetime.now()
